"""Tests for builders/vn_time.py: the deterministic Vietnam operational-time helper.

Proves the fix for the host-OS-dependent `datetime.now(timezone.utc).astimezone()` pattern:
explicit +07:00 offset, no DST, fixed-instant determinism, and independence from whatever
timezone the executing host happens to be in.
"""
    ...
    def test_changing_real_process_timezone_leaves_vn_offset_unchanged(self):
        ...
        try:
            ...
            for zone in ("America/New_York", "UTC", "Asia/Kolkata", "Pacific/Auckland"):
                ...
                old_offsets.add(datetime.now(timezone.utc).astimezone().utcoffset())
