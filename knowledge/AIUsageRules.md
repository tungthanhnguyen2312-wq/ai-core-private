...
- Với `financial_analysis_consumer_context/v1`, chỉ dùng trạng thái và bằng chứng định tính do Producer `financial_analysis_compact/v1` phát hành; không tính TTM, P/E, P/S, growth, margin, hay CFO/NI từ thành phần, không bịa numeric TTM, score, ranking, valuation, forecast hoặc recommendation. Giữ riêng profitability, growth, margin trajectory, cash conversion, turnaround và valuation availability — không gộp thành một điểm số. `RESEARCH_PROXY` (đặc biệt `CROSS_PROVIDER_UNRESOLVED_SCALE`) chỉ là research context định hướng, không phải fact có thẩm quyền; `ABSENT`/`BLOCKED`/`UNAVAILABLE` là giới hạn coverage, còn `NOT_APPLICABLE` là giới hạn áp dụng — không được diễn giải thành yếu, bằng không, hoặc trung tính.
- Financial V2 chỉ giải thích bằng chứng đã có: `research_stance` không phải execution order, và Tactical Entry Readiness không phải Security Research Stance. Không dùng các trường TTM/turnaround/valuation mới để retune, override, upgrade hoặc downgrade `research_stance`, `entry_state`, `entry_action`, confirmation/invalidation, portfolio fit, hay research queue priority của Producer. `RESEARCH_USABLE` là bằng chứng định giá research hiện hành; `INPUT_BLOCKED` là method bị chặn/không dùng được, không phải ước lượng số; `PE_NOT_MEANINGFUL` không phải P/E rẻ/đắt; `TTM_MARKET_CAP_MONETARY_BASIS_INCOMPATIBLE` là blocker cơ sở tiền tệ tường minh.
